...
function bindEvents() {
	...
	document.addEventListener("keydown", (e) => {
		if (
			e.key === "Escape" &&
			!elements.classModal.classList.contains("hidden")
		) {
			e.preventDefault();
			hideClassModal();
		}
...
function bindCallbacks() {
	...
	fileManager.onImagesLoaded = (images, _dir) => {
		...
		if (images.length > 0) {
			...
			const indexToLoad =
				...
				fileManager.currentIndex < images.length
	...
	fileManager.onImageChanged = async (image, index, imageData) => {
		...
		canvas.loadYoloAnnotations(
			labels,
			classManager.classes.map((c) => classManager.getClassName(c.id)),
		);
...
function renderFileList(images) {
	elements.fileList.innerHTML = images
		.map(
			(img, index) => `
...
function updateLabelList() {
	...
	elements.labelList.innerHTML = annotations
		.map(
			(ann, index) => `
...
function updateUIState() {
	...
	elements.btnNext.disabled =
		!hasImages || fileManager.currentIndex >= fileManager.images.length - 1;
...
function updateModalClassList(classes) {
	elements.modalClassList.innerHTML = classes
		.map(
			(cls) => `
    <li data-id="${cls.id}">
      <span class="class-color" style="background: ${cls.color}"></span>
...
function selectClassForPendingRect(classId) {
	...
	if (state.pendingAnnotationIndex >= 0) {
		canvas.updateAnnotationClass(
			state.pendingAnnotationIndex,
			classId,
			className,
		);
		hideClassModal();
...
function updateSplitDisplay() {
	...
	// 隐藏太窄的标签
	elements.splitTrain.querySelector(".split-segment-label").style.display =
		trainRatio < 8 ? "none" : "";
	elements.splitVal.querySelector(".split-segment-label").style.display =
		valRatio < 8 ? "none" : "";
	...
	function onHandleMouseDown(e) {
		...
		const onMouseMove = (moveEvent) => {
			...
			if (exportState.draggingHandle === 1) {
				// 拖拽第一个 handle：调整 train 和 val 的分界
				let newTrain = Math.round(pct);
				newTrain = Math.max(
					MIN_RATIO,
					Math.min(100 - exportState.testRatio - MIN_RATIO, newTrain),
				);
				...
				// 拖拽第二个 handle：调整 val 和 test 的分界
				let newTest = Math.round(100 - pct);
				newTest = Math.max(
					MIN_RATIO,
					Math.min(100 - exportState.trainRatio - MIN_RATIO, newTest),
				);
